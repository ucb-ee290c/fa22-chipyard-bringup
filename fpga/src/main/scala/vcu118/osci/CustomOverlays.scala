...
import chipyard.fpga.vcu118.{FMCPMap, FMCMap}

/* Connect the I2C to certain FMC pins */
class OsciI2CVCU118PlacedOverlay(val shell: VCU118ShellBasicOverlays, name: String, val designInput: I2CDesignInput, val shellInput: I2CShellInput)
...
class OsciI2CVCU118ShellPlacer(val shell: VCU118ShellBasicOverlays, val shellInput: I2CShellInput)(implicit val valName: ValName)
  extends I2CShellPlacer[VCU118ShellBasicOverlays]
{
  def place(designInput: I2CDesignInput) = new OsciI2CVCU118PlacedOverlay(shell, valName.name, designInput, shellInput)
}

/* Connect the UART to certain FMC pins */
class OsciUARTVCU118PlacedOverlay(val shell: VCU118ShellBasicOverlays, name: String, val designInput: UARTDesignInput, val shellInput: UARTShellInput)
...
class OsciUARTVCU118ShellPlacer(shell: VCU118ShellBasicOverlays, val shellInput: UARTShellInput)(implicit val valName: ValName)
  extends UARTShellPlacer[VCU118ShellBasicOverlays] {
  def place(designInput: UARTDesignInput) = new OsciUARTVCU118PlacedOverlay(shell, valName.name, designInput, shellInput)
}
...
class OsciGPIOVCU118PlacedOverlay(val shell: VCU118ShellBasicOverlays, name: String, val designInput: GPIODesignInput, val shellInput: GPIOShellInput, gpioNames: Seq[String])
   extends GPIOXilinxPlacedOverlay(name, designInput, shellInput)
{
  shell { InModuleBody {
    ...
    val packagePinsWithIOStdWithPackageIOs = (gpioNames zip io.gpio).map { case (name, io) =>
      val (pin, iostd, pullupEnable) = OsciGPIOs.pinMapping(name)
...
class OsciGPIOVCU118ShellPlacer(shell: VCU118ShellBasicOverlays, val shellInput: GPIOShellInput, gpioNames: Seq[String])(implicit val valName: ValName)
  extends GPIOShellPlacer[VCU118ShellBasicOverlays] {
  def place(designInput: GPIODesignInput) = new OsciGPIOVCU118PlacedOverlay(shell, valName.name, designInput, shellInput, gpioNames)
}
...
class TSIHostVCU118PlacedOverlay(val shell: OsciVCU118FPGATestHarness, name: String, val designInput: TSIHostDesignInput, val shellInput: TSIHostShellInput)
...
class OsciTSIHostVCU118PlacedOverlay(override val shell: OsciVCU118FPGATestHarness, override val name: String, override val designInput: TSIHostDesignInput, override val shellInput: TSIHostShellInput)
  extends TSIHostVCU118PlacedOverlay(shell, name, designInput, shellInput)
{
  ...
  shell { InModuleBody {
    ...
    // require(di.serialIfWidth == 4) // remnant from beagle chip bringup
    ...
    val packagePinsWithPackageIOs = Seq(
      (FMCMap("G7"), clkIo),
      (FMCMap("G10"), IOPin(io.serial.out.ready)),
      (FMCMap("G13"), IOPin(io.serial.out.valid)),
      (FMCMap("G16"), IOPin(io.serial.out.bits, 0)),
      (FMCMap("G19"), IOPin(io.serial.in.ready)),
      (FMCMap("G22"), IOPin(io.serial.in.valid)),
      (FMCMap("G25"), IOPin(io.serial.in.bits, 0)),
      )
...
class OsciTSIHostVCU118ShellPlacer(shell: OsciVCU118FPGATestHarness, val shellInput: TSIHostShellInput)(implicit val valName: ValName)
  extends TSIHostShellPlacer[OsciVCU118FPGATestHarness] {
  def place(designInput: TSIHostDesignInput) = new OsciTSIHostVCU118PlacedOverlay(shell, valName.name, designInput, shellInput)
}
